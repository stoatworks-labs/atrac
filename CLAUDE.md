...
- No OpenFX port, no browser demo, no factory presets, no user guide (`guide=""` in
  the About block).
- `StoatworksAbout.h` and `ATTRIBUTIONS.md` are provisional hand copies; register the
  project and re-run stoatworks-backend's syncs before the first release.
