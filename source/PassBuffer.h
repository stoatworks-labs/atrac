...
/**
    ...
    **It reallocates only when it has to.** Ensure() is called every frame and
    ...
    host's picture changes size or Block Size changes the coefficient
    layout. A reallocation clears. The one buffer here that outlives a frame
    is the held picture the disc shows while it is muted, and that one is
    deliberately NOT re-ensured while the mute lasts (see `Atrac.cpp` and
    `actest --resize`).
    ...
    **It owns its filtering.** Every coefficient buffer is read texel for
    texel with `texelFetch`, so they are Nearest. The held picture is read
    with normalised coordinates (a stale size still has to map onto the
    output), so it is Linear. Mipmapped is tinsel's and is unused.
