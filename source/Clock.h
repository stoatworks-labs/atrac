...
/**
	...
	from whatever the host hands SetTime. Copied from clamp (by way of standards) unchanged.
	...
	**Resolume's clock overflows a float.** It has been measured at ~499
	million ms, where a float resolves ~0.03 s -- two frames. So the
	clock is kept as an origin and an offset in double, the buffer's
	arithmetic is done in double (`Disc.cpp`) and nothing absolute ever
	reaches a shader.

	**The disc runs on elapsed time.** A delta is believed unless it is
	backwards or longer than half a second -- a scrub, a loop, a stall -- in
	which case the clock steps on by one nominal frame and carries on from
	there. The buffer sees a continuous clock either way, and a
	half-second stall does not drain it.
*/
namespace atrac
{
class Clock
