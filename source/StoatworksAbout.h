/*
 * Stoatworks Labs - About window data for Atrac.
 *
 * PROVISIONAL hand copy (2026-09-24), adapted from clamp's generated header.
 * The real one is GENERATED by stoatworks-backend/scripts/sync-about.py from
 * the website's projects.json once the project is registered there; until
 * then this is what the About block shows. `guide` is empty on purpose: no
 * user guide exists yet, and an empty link is left out of the button list
 * rather than shown as a button that opens a 404 (see StoatworksAboutLinks.h).
 *
 * `version` here is a fallback read from this repo's own manifest at sync
 * time. Anything with a build step injects the real one at build time and
 * overrides this.
 */
#pragma once

namespace stoatworks::about
{
    inline constexpr auto name = "Atrac";
    inline constexpr auto slug = "atrac";
    inline constexpr auto hook = "MiniDisc's perceptual coder, run on the picture, for Resolume";
    inline constexpr auto licence = "MIT";
    inline constexpr auto guide = "";
    inline constexpr auto page = "https://stoatworks-labs.com/software/atrac/";
    inline constexpr auto repo = "https://github.com/stoatworks-labs/atrac";
    inline constexpr auto versionFallback = "v0.1.0";

    inline constexpr auto org = "Stoatworks Labs";
    inline constexpr auto home = "https://stoatworks-labs.com";
    inline constexpr auto tagline = "Open tools for the people who run the show.";

    /* The canonical funding links, matching FUNDING.yml and the support footer. */
    struct Link { const char* name; const char* url; };
    inline constexpr Link funding[] = {
        { "GitHub Sponsors", "https://github.com/sponsors/stoatworks-labs" },
        { "Ko-fi", "https://ko-fi.com/stoatworkslabs" },
        { "Patreon", "https://patreon.com/StoatworksLabs" },
        { "Liberapay", "https://liberapay.com/stoatworks-labs" },
    };
}
