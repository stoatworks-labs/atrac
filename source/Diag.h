...
/**
    ...
    **A shader that will not compile.** `InitGL` returns `FF_FAIL` and from
    ...
    message anywhere; with ten passes this also records *which* one, which is
    ...
    **A buffer that could not be allocated.** This plugin holds five
    float RGBA buffers the size of the picture or its padded coefficient
    plane, and the failure mode when the driver says no is a black frame
    with nothing to explain it. What goes in the log is the size asked for:
    at 4K the set comes to about 660 MB of float texture.
*/
namespace atrac::diag
{
