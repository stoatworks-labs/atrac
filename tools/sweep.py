...
test card and reports any that made no difference at all.
    ...
    python3 tools/sweep.py [--binary build/actest] [--size WxH] [--jobs N] [--allow-no-gl]

Exit code 1 means something is dead.

------------------------------------------------------------------ the traps

**The Disc group only shows through a knock.** Buffer, Read Speed and Knock
Length change nothing until a knock has drained the buffer; Sensitivity
changes nothing until audio arrives. CONTEXT presses the Knock button for
them (it is an event: `--set "Knock=1"` is one rising edge on frame 0), sets
the others so that the two ends of the swept control land one side each of
the break-or-not or resumed-or-not boundary at the last frame rendered, and
feeds the harness's synthetic spectrum for Sensitivity (`_args`). The frame
counts are chosen from the disc's closed form (AGENTS.md) with a margin of
at least two frames each side.

**Knock itself needs an empty buffer**, or the default two seconds absorb it.

**An option's range is its element count.** An option parameter's range reads
back 0..1 whatever its element count, so `actest --list` prints the count as
...
#   _args       extra harness arguments
#
# Read Speed: Buffer 0.5 s (v 0.05), Knock Length 2 s (v 0.8). The knock at
# frame 0 breaks playback at 0.5 s and the read comes back at 2 s; the mute
# ends when the buffer refills to 0.125 s: at 1x that is frame 127.5, at 4x
# frame 121.9. Frame 125 sits between them.
CONTEXT = {
    "Knock": {"Buffer": 0, "Knock Length": 1},
    "Buffer": {"Knock": 1, "Knock Length": 1},
    "Knock Length": {"Knock": 1, "Buffer": 0},
    "Read Speed": {"Knock": 1, "Buffer": 0.05, "Knock Length": 0.8, "_frames": 125},
    "Sensitivity": {"Buffer": 0, "Knock Length": 1, "_args": ["--feed"]},
}
...
def main():
    ...
    ap.add_argument("--binary", default=str(ROOT / "build" / "actest"))
    ...
    scratch = tempfile.mkdtemp(prefix="acsweep")
