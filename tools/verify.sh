...
#                 strings `actest --dump-shaders` writes.
#   offline       the checks that need no GL, and their negative controls:
#                   --tables    the BFU rule, the scale-factor law, the window
#                               and basis, the stated transform's own
#                               reconstruction in double, the control laws
#                   --names     nothing a host will silently truncate; the
#                               host reads SW Atrac / AC01 / effect
#                   --skipmodel the disc's arithmetic against the closed form
#                   --prime     no knock on the first frame of a loud clip
#   physics       every rendering check, at TWO rasters: 320x180, which is
#                 what CI renders at, and 1280x720. Each is measured out of
#                 the picture or the plugin's own allocation texture:
#                   --tdac      unlimited bits: the transform reconstructs the
#                               input within a derived l2 bound, through
#                               long, short and mixed blocks
#                   --budget    every cell spends at most its budget and
#                               wastes less than one increment
#                   --preecho   a step's noise reaches N before it in Long,
#                               3N/8 in Short and Adaptive
#                   --snr       SNR on the rate card rises with Bit Rate
#                   --masking   a masked band is zeroed, coded with Masking off
#                   --skip      a knock breaks playback iff L > B, resuming on
#                               the closed form
#                   --resize    a resize mid-mute keeps the held picture
...
step "shaders"
if out=$(tools/check-shaders.sh "$ACTEST" 2>&1); then
	pass "$( printf '%s\n' "$out" | tail -1 | sed 's/^ *//' )"
else
...
step "offline (no GL)"
for check in tables names skipmodel prime negative-offline; do
	if out=$("$ACTEST" --$check 2>&1); then
...
for size in 320x180 1280x720; do
	...
	for check in tdac budget preecho snr masking skip resize negative; do
		if out=$("$ACTEST" --$check --size $size 2>&1); then
			pass "actest --$check: $( printf '%s\n' "$out" | grep -v '^$' | grep -v 'checks,' | tail -1 )"
		else
			fail "actest --$check at $size"
			printf '%s\n' "$out" | sed 's/^/      /'
		fi
	done
done
...
got=$( "$ACTEST" --pipe --size 64x36 < "$raw" 2>/dev/null | wc -c | tr -d ' ' )
status=${PIPESTATUS[0]}
...
# pipeline whatever actest did, and the refusal would pass for the wrong reason.
printf '0 No Such Control 0.5\n' > "$cues"
"$ACTEST" --pipe --size 64x36 --script "$cues" < "$raw" >/dev/null 2>&1
status=$?
if [ "$status" -eq 2 ]; then
...
got=$( "$ACTEST" --pipe --size 64x36 --fail-render-at 1 < "$raw" 2>/dev/null | wc -c | tr -d ' ' )
status=${PIPESTATUS[0]}
...
# A reader that goes away after one byte: forty frames is far more than a pipe
# buffer holds, so the writes after head leaves must fail. Exit 1, said on
# stderr -- not the 141 of a process SIGPIPE killed before it could say anything.
"$ACTEST" --pipe --size 64x36 < "$many" 2>/dev/null | head -c 1 >/dev/null
status=${PIPESTATUS[0]}
if [ "$status" -eq 1 ]; then
...
"$ACTEST" --bench --frames 60 2>&1 | sed -n '3,6p' | sed 's/^/   /'

BUNDLE="$BUILD/Atrac.bundle"
BIN="$BUNDLE/Contents/MacOS/Atrac"
...
if [ "$(uname)" = "Darwin" ] && [ -d "$BUNDLE" ]; then
	...
	if [ "$ident" = "com.stoatworks.ffgl.atrac" ]; then
	...
	if codesign --force --sign - --timestamp=none "$tmp/Atrac.bundle" >/dev/null 2>&1; then
	...
	if [ -x "$OXBOW" ]; then
		...
		for want in "name:        SW Atrac" "id:          AC01" "type:        effect"; do
			case "$probe" in
